...
<script setup>
  ...
  const { el, value, label, hidden, unstyled, size, color, visual, icon, iconAfter } = toRefs(props)
...
.button[visual="secondary"]:focus { background-image: linear-gradient(hsla(var(--_button-hsl), 0.1), hsla(var(--_button-hsl), 0.1)); }
...
.button[data-icon]:before,
.button[icon]:before { content: attr(icon); }
...
.button[data-icon]:before,
.button[icon]:before { margin-right: var(--s-1); }

.button[data-icon-after]:after,
.button[icon-after]:after   { margin-left: var(--s-1); }

.button[data-icon]:empty:before,
.button[icon]:empty:before { margin-right: 0; }
...
.button[icon-after]:empty { padding: var(--_button-padding-v) var(--_button-padding-v); }
